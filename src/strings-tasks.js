...
function getStringLength(value) {
  let stringLength = 0;
  if (typeof value === 'string') stringLength = value.length;
  return stringLength;
}
...
function concatenateStrings(value1, value2) {
  const resultConcatenate = value1.concat(value2);
...
function getFirstChar(value) {
  const resultFirstChar = value.charAt(0);
  return resultFirstChar;
}
...
function removeLeadingAndTrailingWhitespaces(value) {
  const resultRemoveTrailing = value.trim();
...
function repeatString(str, times) {
  let resultRepeat = '';
  if (times < 0) return resultRepeat;
  resultRepeat = str.repeat(times);
  return resultRepeat;
}
...
function removeFirstOccurrences(str, value) {
  const indexOccurrence = str.indexOf(value);
  if (indexOccurrence !== -1) {
    const beforeOccurrence = str.slice(0, indexOccurrence);
    const afterOccurrence = str.slice(indexOccurrence + value.length);
    const resultStr = beforeOccurrence + afterOccurrence;
    return resultStr;
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const indexOccurrence = str.lastIndexOf(value);
  if (indexOccurrence !== -1) {
    const beforeOccurrence = str.slice(0, indexOccurrence);
    const afterOccurrence = str.slice(indexOccurrence + value.length);
    const resultStr = beforeOccurrence + afterOccurrence;
...
function startsWith(str, substr) {
  const resultStartsWith = str.startsWith(substr);
...
function endsWith(str, substr) {
  const resultEndsWith = str.endsWith(substr);
...
function formatTime(minutes, seconds) {
  const resultFormatTime = `${String(minutes).padStart(2, '0')}:${String(
    seconds
  ).padStart(2, '0')}`;
  return resultFormatTime;
}
...
function countVowels(str) {
  ...
  let resultCountVowels = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str[i])) resultCountVowels += 1;
  }
  return resultCountVowels;
}
...
function isPalindrome(str) {
  ...
  const onlyLetterString = lowerCaseString.replace(/[^a-z]/g, '');
  const reverseStr = onlyLetterString.split('').reverse().join('');
  const stringIsPalindrome = onlyLetterString === reverseStr;
...
function reverseWords(str) {
  ...
  const resultReverseWords = reversedWordsArray.join(' ');
...
function invertCase(str) {
  ...
  const invertCaseArray = array.map((character) => {
    if (character === character.toUpperCase()) return character.toLowerCase();
    return character.toUpperCase();
  });
  const resultInvertCase = invertCaseArray.join('');
  return resultInvertCase;
}
...
function getStringFromTemplate(firstName, lastName) {
  const resultString = `Hello, ${firstName} ${lastName}!`;
  return resultString;
}
...
function extractNameFromTemplate(value) {
  const resultName = value.slice(7, -1);
  return resultName;
}
...
function unbracketTag(str) {
  const resultUnbracketTag = str.slice(1, -1);
  return resultUnbracketTag;
}
...
function extractEmails(str) {
  const resultArrayEmails = str.split(';');
  return resultArrayEmails;
}
